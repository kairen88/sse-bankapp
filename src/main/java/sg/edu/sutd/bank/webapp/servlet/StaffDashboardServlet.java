...
public class StaffDashboardServlet extends DefaultServlet {
	...
	private void onRegistrationDecisionAction(HttpServletRequest req, HttpServletResponse resp)
		...
		if (!users.isEmpty()) {
			try {
				userDAO.updateDecision(users); //HOW TO SYNC OR LOCK HERE?
			} catch (ServiceException e) {
				sendError(req, e.getMessage());
			}
			...
			try {
				...
				{				
					synchronized(Locks.transactionLock) {
						...
						if(clientTrans != null && clientTrans.getStatus().compareTo(TransactionStatus.APPROVED) == 0) 
						{
							...
							throw new ServiceException(new Throwable("Transaction is invalid"));
						}
					}
				}
			} catch (ServiceException e) {
